...
public class IntraVpnService extends VpnService implements NetworkManager.NetworkListener,
  ...
  private static final int SERVICE_ID = 1; // Only has to be unique within this net.
  ...
  public void onSharedPreferenceChanged(SharedPreferences preferences, String key) {
    if (PersistentState.APPS_KEY.equals(key) && vpnAdapter != null) {
      // Restart the VPN so the new net exclusion choices take effect immediately.
      restartVpn();
    }
  ...
  public synchronized int onStartCommand(Intent intent, int flags, int startId) {
    ...
    // which helps users understand what's going on, and return to the net if they want.
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      // Secret notifications are not shown on the lock screen.  No need for this net to show there.
  ...
  public VpnService.Builder newBuilder() {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
      ...
      try {
        // Workaround for any net incompatibility bugs.
        ...
        Log.e(LOG_TAG, "Failed to exclude an net", e);
      }
    }
    return builder;
  }
